...
print ("welcome, Created By anonymous\n")

print ("Please subscribe to the channel to receive updates! : for telegram \n")
				...
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
							...
							try:
								bot.sendMessage(target, "متنی وجود ندارد!", message_id=msg.get("message_id"))
							except:
								print("err admin")
						...
						elif msg.get("text").startswith("تست") or msg.get("text").startswith("لیست اشکال") or msg.get("text").startswith("صلام") or msg.get("text").startswith("صلم") or msg.get("text").startswith("hi") or msg.get("text").startswith("Hi") or msg.get("text").startswith("Hello") or msg.get("text").startswith("hello"):
							try:
								bot.sendMessage(target, "درحال حاضر مشکلی وجود ندارد", message_id=msg.get("message_id"))
							except:
								print("err answer hello")
								
						elif msg.get("text").startswith("سازنده ربات") or msg.get("text").startswith("خبی"):
							try:
								bot.sendMessage(target, "اطلاعاتی از سازندم در دسترس نیست", message_id=msg.get("message_id"))
							except:
								print("err answer hay")
								
						elif msg.get("text").startswith("چقدر عجیب") or msg.get("text").startswith("چخبر"):
							try:
								bot.sendMessage(target, "جالب شد", message_id=msg.get("message_id"))
							except:
								print("err CheKhabar")
								
						elif msg.get("text").startswith("ربات") or msg.get("text").startswith("بات"):
							try:
								bot.sendMessage(target, "درخدمتم", message_id=msg.get("message_id"))
							except:
								print("err bot answer")
								
						elif msg.get("text").startswith("عجب") or msg.get("text").startswith("🤣"):
							try:
							...
							try:
								bot.sendMessage(target, "مث اینکه داری به یکی داری میگی دلقک نه؟", message_id=msg.get("message_id"))
							except:
							...
							try:
								...
								bot.sendMessage(target, "نتیجه با موفقیت ارسال شد✅", message_id=msg["message_id"])
							except:
								bot.sendMessage(target, "دستور رو درست وارد کن  کاربر گرامی❗️", message_id=msg["message_id"])
							...
							try:
								...
								bot.sendMessage(target, "دستور درست نمیباشد لطفا یک بار دیگر تلاش کنید❗️", message_id=msg["message_id"])
							...
							try:
								...
								bot.sendMessage(target, "شرمنده  دستور با موفقیت اجرا نشد بار دیگر سعی کنید", message_id=msg["message_id"])
							...
							try:
								...
								bot.sendMessage(target, "دستور رو درست وارد کنید ❗️", message_id=msg["message_id"])
						...
						elif msg.get("text") == "تایمر" and msg.get("author_object_guid") in admins:
							try:
								number = 10
						...
						elif msg.get("text") == "غیرفعال کردن تایمر" and msg.get("author_object_guid") in admins:
							try:
								number = 0
								bot.setGroupTimer(target,number)
